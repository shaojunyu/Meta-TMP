...
# Last update: 2016-03-22, Xiaoquan Su
...
option_list <- list(
  ...
  make_option(c("-o", "--out_dir"), type="character", default='Alpha_diversity', help="Output file name[default %default]"),
  make_option(c("-p", "--prefix"), type="character",default='Out', help="Output file prefix [Optional, default %default]")
)
opts <- parse_args(OptionParser(option_list=option_list), args=args)

# paramenter checking
if(is.null(opts$table_file)) stop('Please supply an input table.')
if(is.null(opts$meta_data)) stop('Please supply a meta-data table.')
...
#write.table("Alpha diversity wilcoxon test\nPairwise.wilcox.test FDR",file="Alpha_diversity_wilcoxon.txt",quote=F,,row.names=F,col.names = F)
pdf(outpdffile,height = 10,width=f*0.3*j)
layout(matrix(1:(j*3),3,j,byrow = T),widths=x)
#invisible(write.table("\n=====================\nShannon:",file="Alpha_diversity_wilcoxon.txt",quote=F,,row.names=F,col.names = F,append = T,eol="\n\t"))
for (i in 2:ncol(data_map)) {
  if (is.numeric(data_map[1,i])==F) {tempframe_shannon=data.frame(shan=c(indexes[,1]),dv=as.factor(data_map[,i])) #to regenerate a dataframe with discrete variable as last col                                                       
                                     attach(tempframe_shannon)
                                     suppressWarnings(invisible(shant <- pairwise.wilcox.test(shan,dv,p.adjust="fdr")))
#                                    write.table(shant$p.value,file="Alpha_diversity_wilcoxon.txt",quote=F,row.names=T,col.names = T,append = T,sep = "\t")
#                                    write.table("",file="Alpha_diversity_wilcoxon.txt",quote=F,row.names=F,col.names = F,append = T,eol = "\t")
                                     invisible(if (nlevels(dv)==2) (mt=shant$p.value) else (mt=(kruskal.test(shan, dv))$p.value))
                                     boxplot(shan ~ dv, data = tempframe_shannon,log = "y", xlab= colnames(data_map)[i],ylab="Shannon",main=paste("P=",round(mt,digits=4)),cex.main=1,col=colours)
...
#invisible(write.table("\n=====================\nSimpson:",file="Alpha_diversity_wilcoxon.txt",quote=F,,row.names=F,col.names = F,append = T,eol="\n\t"))
for (i in 2:ncol(data_map)) {
  if (is.numeric(data_map[1,i])==F) {tempframe_simpson=data.frame(shan=c(indexes[,2]),dv=as.factor(data_map[,i])) #to regenerate a dataframe with discrete variable as last col              
                                     attach(tempframe_simpson)
                                     suppressWarnings(invisible(simpt <- pairwise.wilcox.test(shan,dv,p.adjust="fdr")))
#                                    write.table(simpt$p.value,file="Alpha_diversity_wilcoxon.txt",quote=F,row.names=T,col.names = T,append = T,sep = "\t")
#                                    write.table("",file="Alpha_diversity_wilcoxon.txt",quote=F,row.names=F,col.names = F,append = T,eol = "\t")
                                     invisible(if (nlevels(dv)==2) (mt=simpt$p.value) else (mt=(kruskal.test(shan, dv))$p.value))
                                     boxplot(shan ~ dv, data = tempframe_simpson,log = "y", xlab= colnames(data_map)[i],ylab="Simpson",main=paste("P=",round(mt,digits=4)),cex.main=1,col=colours)
...
#invisible(write.table("\n=====================\nCHAO1:",file="Alpha_diversity_wilcoxon.txt",quote=F,,row.names=F,col.names = F,append = T,eol="\n\t"))
for (i in 2:ncol(data_map)) {
  if (is.numeric(data_map[1,i])==F) {tempframe_chaoooo=data.frame(shan=c(indexes[,3]),dv=as.factor(data_map[,i])) #to regenerate a dataframe with discrete variable as last col        
                                     attach(tempframe_chaoooo)
                                     suppressWarnings(invisible(chaot <- pairwise.wilcox.test(shan,dv,p.adjust="fdr")))
#                                     write.table(chaot$p.value,file="Alpha_diversity_wilcoxon.txt",quote=F,row.names=T,col.names = T,append = T,sep = "\t")
#                                     write.table("",file="Alpha_diversity_wilcoxon.txt",quote=F,row.names=F,col.names = F,append = T,eol = "\t")
                                     invisible(if (nlevels(dv)==2) (mt=chaot$p.value) else (mt=(kruskal.test(shan, dv))$p.value))
                                     boxplot(shan ~ dv, data = tempframe_chaoooo,log = "y", xlab= colnames(data_map)[i],ylab="CHAO1",main=paste("P=",round(mt,digits=4)),cex.main=1,col=colours)
