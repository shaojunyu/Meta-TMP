...
# Last update: 2015-06-08, Yanhai Gong, Xiaoquan Su
#################################################################
...
PM_heatmap <- function(da,clust_method="average") {
  ...
  pdf(opts$outfile,width=14+attr(dist.data,'Size')/5,height=14+attr(dist.data,'Size')/5)
  
  suppressWarnings(suppressMessages(heatmap.2(as.matrix(dist.data),scale="none",hclustfun=myclust,key=TRUE,key.par=list(cex.axis=2),symkey=FALSE,
            density.info="none",trace="none",col=colorpanel(100,low="red",high="green"),
            lmat=rbind( c(0, 3), c(2,1), c(0,4) ), lhei=c(1.5, 4, 0.5 ))))
