...
# Last update: 2016-04-22, Yanhai Gong, Xiaoquan Su
#################################################################
...
option_list <- list(  
  ...
  #make_option(c("-a", "--axesfile"), type="character", default='pcoa_axes.txt', help="Output PCA axes [default %default]")
)
opts <- parse_args(OptionParser(option_list=option_list), args=args)

# paramenter checking
if(is.null(opts$meta_data)) stop('Please supply a meta-data file.')
if(is.null(opts$dist_file)) stop('Please supply a distance matrix table.')
...
PM_pcoa <- function(da, md, method="bray") {
  ...
  for (i in 1:n) {
    ...
    if (opts$drawlabel) {
      pcoa12<-ggplot() +
        geom_point(data=data,aes(x=PC1,y=PC2,color=group,shape=group,label=group2,ymax=max(PC2)),stat='identity',position='stack',size=4) + 
        geom_text(data=data,aes(x=PC1,y=PC2,label=group2),hjust=0,vjust=-1,alpha=0.8) +
        scale_colour_manual(values=gcol) + #scale_shape_manual(values=gshape) +
        guides(col=guide_legend(colnames(meta)[i],ncol=ceiling(gnum/14))) +
      ...
      pcoa13<-ggplot() +
        geom_point(data=data,aes(x=PC1,y=PC3,colour=group,label=group2,ymax=max(PC3)),stat='identity',position='stack',size=4) +
      ...
      pcoa23<-ggplot() +
        geom_point(data=data,aes(x=PC2,y=PC3,colour=group,label=group2,ymax=max(PC3)),stat='identity',position='stack',size=4) +
      ...
      pcoa12<-ggplot() +
        geom_point(data=data,aes(x=PC1,y=PC2,color=group,label=group2,ymax=max(PC2)),stat='identity',position='stack',size=4) +
        scale_colour_manual(values=gcol) + #scale_shape_manual(values=gshape) +
        guides(col=guide_legend(colnames(meta)[i],ncol=ceiling(gnum/14))) +
        #  scale_y_continuous(labels=percent) + 
        xlab(paste("PC1 (",loadings[1],"%)")) + ylab(paste("PC2 (",loadings[2],"%)")) + geom_vline(xintercept=0,linetype="longdash") + geom_hline(yintercept=0,linetype="longdash") +
