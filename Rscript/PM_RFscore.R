...
# Call: Rscript PM_RFscore.R -i taxa.abd -m metadata -o outfile
# R packages used: randomForest ggplot2
# Authors: Zheng Sun, Xiaoquan Su
# Last update: 2016-06-01, Xiaoquan Su, Zheng Sun
...
for (i in 2:ncol(data_map)) {
  if (is.numeric(data_map[1,i])==F) {tempframe=data.frame(data_matrix,status=as.factor(data_map[,i])) #to regenerate a dataframe with discrete variable as last col                                                       
  bm.rf <- randomForest(status ~ .,data=tempframe,importance=T,proximity=T,ntree=5000)
  framebm <- data.frame(importance(bm.rf,type = 1))
  IL <- ggplot(framebm,aes(y=reorder(row.names(framebm),framebm$MeanDecreaseAccuracy),x=framebm$MeanDecreaseAccuracy))+geom_point(size=3,colour="#D55E00")+geom_segment(aes(yend=row.names(framebm)),xend=0,colour="#D55E00")+theme_bw()+theme(panel.grid.major.y=element_blank(),panel.grid.major.x=element_blank())+xlab(paste("Mean decrease in accuracy")) + ylab(paste("Features"))+ggtitle("Random-Forest Score")
  ...
  write.table(newframebm[order(newframebm[,1],decreasing=T),][,2:3],file=outtxtfile,quote=F,row.names = F,col.names = F)  
  ...
  suppressMessages(ggsave(filename=outPDFfile,plot=IL,height = (dim(newframebm)[1]*0.15 + 1),limitsize =F))
  } 
}
