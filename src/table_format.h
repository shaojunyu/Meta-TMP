...
void _Table_Format::Calc_Dist_Matrix(const char * outfilename, int metrics, int coren, bool is_sim){
      ...
      //make order
     int * order_m = new int [Samples.size() * (Samples.size()- 1) / 2];
     int * order_n = new int [Samples.size() * (Samples.size() - 1) / 2];
     int iter = 0;
     ...
     for (int i = 0; i < Samples.size() - 1; i ++)
      ...
      //calc dist 
      float * dist_matrix = new float [Samples.size() * Samples.size()];
      memset(dist_matrix, 0, Samples.size() * Samples.size() * sizeof(float));
      ...
      for (int i = 0; i < iter; i ++){
        ...
        if (metrics == 0)
                    dist_matrix[m * Samples.size() + n] = Calc_Dist_Cos(m, n);
        else dist_matrix[m * Samples.size() + n] = Calc_Dist_E(m, n);

        dist_matrix[n * Samples.size() + m] = dist_matrix[m * Samples.size() + n];
        }
       ...
       for (int i = 0; i < Samples.size(); i++){
          outfile <<Samples[i];
          for (int j = 0; j < Samples.size(); j++){
              if (is_sim) outfile << "\t" << 1 - dist_matrix[i * Samples.size() + j];
              else outfile << "\t" << dist_matrix[i * Samples.size() + j];
              }
          outfile <<endl;
          }
...
#pragma omp parallel for schedule(dynamic, 1)
      for (int i = 0; i < iter; i++){
          ...
          else corr_matrix[m * Samples.size() + n] = Calc_Corr_P(m, n);
