...
class _Table_Format_Seq {
 public:
  ...
  void Add_Feature(string feature, int sample, float cp) {
    ...
    Seq_count[sample][feature_index]++;
    Abd_count[sample][feature_index] += 1.0 / cp;
  }

  void Add_Feature(string feature, unsigned count, int sample, float cp) {
    if (Abd_table.Feature_hash.count(feature) == 0) {
      Abd_table.Feature_hash[feature] = Abd_table.Features.size();
      Abd_table.Features.push_back(feature);
  ...
  void Normalization() {
    for (int i = 0; i < Seq_count.size(); i++) {
      ...
      for (int j = 0; j < Abd_count[i].size(); j++) seq_sum += Abd_count[i][j];
  ...
  int Get_Taxa_Size() { return Abd_table.Get_Feature_Size(); }
